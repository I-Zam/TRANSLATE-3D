// TRANSLATE 3D - GitHub Pages Version (FIXED)
// Inventory and sales management system
// Uses CSV export from Google Sheets
...
// Load products from Google Sheets using CSV export
function loadProductsFromSheets() {
  fetch(CSV_URL)
    ...
    .catch(error => {
      ...
      showError('Error cargando productos del Sheet. Verifica que esté compartido públicamente.');
    });
}
...
function parseCSV(csv) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (fields[0] && fields[1]) { // Categoria and Producto
      products.push({
        id: i - 1,
        categoria: String(fields[0]).trim(),
        producto: String(fields[1]).trim(),
        precio: parseInt(fields[2]) || 0,
        cantidad: parseInt(fields[3]) || 0,
        vendidos: parseInt(fields[4]) || 0,
        venta: parseInt(fields[5]) || 0
      });
    }
  }
}
